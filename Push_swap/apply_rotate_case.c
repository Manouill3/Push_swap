...
/*   Updated: 2024/12/03 09:56:04 by mdegache         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_apply_rarb(t_list **stack_a, t_list **stack_b, int c, char s)
{
	if (s == 'a')
	{
		while ((*stack_a)->content != c && ft_find_place(*stack_b, c) > 0)
			rotate_all(stack_a, stack_b);
		while ((*stack_a)->content != c)
			rotate_a(stack_a, 1);
		while (ft_find_place(*stack_b, c) > 0)
			rotate_b(stack_b, 1);
		push_b(stack_a, stack_b);
	}
	else
	{
		while ((*stack_b)->content != c && ft_find_place(*stack_a, c) > 0)
			rotate_all(stack_a, stack_b);
		while ((*stack_b)->content != c)
			rotate_b(stack_b, 1);
		while (ft_find_place(*stack_a, c) > 0)
			rotate_a(stack_a, 1);
